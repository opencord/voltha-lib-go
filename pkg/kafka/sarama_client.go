...
func (sc *SaramaClient) createPublisher(ctx context.Context) error {
	...
	// WaitForAll (acks=all) → required for strong durability ideal for production setup.
	if sc.requiredAcks == sarama.WaitForAll {
		// Idempotent = true → prevents duplicates on retries
		config.Producer.Idempotent = true
		// MaxOpenRequests = 1 → required for idempotent producer in Sarama (to preserve ordering)
