...
func LivenessInterval(opt time.Duration) SaramaClientOption {
	return func(args *SaramaClient) {
		args.livenessInterval = opt
	}
}
...
func NewSaramaClient(opts ...SaramaClientOption) *SaramaClient {
	...
	client.livenessInterval = DefaultLivenessInterval
...
func (sc *SaramaClient) updateLiveness(alive bool) {
	...
	// events to the channel is rate-limited by livenessInterval.
	if sc.liveness != nil {
		if sc.alive != alive {
			log.Info("update-liveness-channel-because-change")
			sc.liveness <- alive
			sc.lastLivenessTime = time.Now()
		} else if time.Now().Sub(sc.lastLivenessTime) > sc.livenessInterval {
