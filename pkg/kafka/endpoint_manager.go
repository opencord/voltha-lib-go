...
func (ep *endpointManager) GetEndpointByService(deviceID string, serviceID string) (Endpoint, error) {
	logger.Debugw("GetEndpointByService", log.Fields{"device-id": deviceID, "service-id": serviceID})
	...
	if m.GetEndPoint() == "" {
		return "", status.Errorf(codes.Unavailable, "endpoint-not-set-%s", serviceID)
	}
...
func (ep *endpointManager) loadServices() error {
	...
	for key, val := range ep.services {
		...
		for _, m := range members {
			n := m.(Member)
			logger.Debugw("service-loaded", log.Fields{"serviceId": n.GetServiceID(), "replica": n.GetReplica(), "endpoint": n.GetEndPoint()})
		}
	}
