...
func (ep *EPTest) populateBackend() error {
	...
	adapterID := "adapter_brcm_openomci_onu"
	...
	for i := 0; i < numReplicas; i++ {
		adapter := &voltha.Adapter{
			...
			Endpoint:       fmt.Sprintf("%s_%d", adapterID, i),
		}
		adapterKVKey := fmt.Sprintf("%s/%d", adapterID, i)
...
func (ep *EPTest) testEndpointManagerAPIs(t *testing.T, tm EndpointManager, serviceID string, deviceType string, replicas int) {
	...
	for i := 0; i < numDevices; i++ {
		...
		endpoint, err := tm.GetEndpoint(deviceID, deviceType)
		if err != nil {
			logger.Fatalw("error-getting-endpoint", log.Fields{"error": err})
		}
		deviceIDs[deviceID] = endpoint
		replicaID, err := tm.getReplicaAssignment(deviceID, serviceID)
		if err != nil {
			logger.Fatalw("error-getting-endpoint", log.Fields{"error": err})
		}
		total[replicaID] += 1
	...
	fmt.Println(fmt.Sprintf("Device distributions => devices:%d service_replicas:%d mean:%d standard_deviation:%d", numDevices, replicas, int(mean), int(sdtDev)))
	...
	for i := 0; i < numIterations; i++ {
		for deviceID, expectedEndpoint := range deviceIDs {
			endpointByDeviceType, err := tm.GetEndpoint(deviceID, deviceType)
			if err != nil {
				logger.Fatalw("error-getting-endpoint", log.Fields{"error": err})
			}
			endpointByServiceID, err := tm.GetEndpointByService(deviceID, serviceID)
			if err != nil {
				logger.Fatalw("error-getting-endpoint", log.Fields{"error": err})
			}
			assert.Equal(t, expectedEndpoint, endpointByDeviceType)
			assert.Equal(t, expectedEndpoint, endpointByServiceID)
	...
	for deviceID := range deviceIDs {
		...
		for k := 0; k < replicas; k++ {
			owned, err := tm.IsDeviceOwnedByService(deviceID, serviceID, int32(k))
			if err != nil {
