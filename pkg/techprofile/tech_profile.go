...
const (
	MaxUniPortPerUni = 16 // TODO: Adapter uses its own constant for MaxUniPort. How to synchronize this and have a single source of truth?
)
...
func (t *TechProfileMgr) GetTPInstance(ctx context.Context, path string) (interface{}, error) {
	...
	default:
		...
		return nil, fmt.Errorf("unknown-tech-%s--tp-path-%v", tech)
	}
}

func (t *TechProfileMgr) CreateTechProfInstance(ctx context.Context, tpID uint32, uniPortName string, intfID uint32) (interface{}, error) {
	...
	if !uniPortNameFormatRegexp.Match([]byte(uniPortName)) {
		...
		return nil, errors.New(fmt.Sprintf("uni-port-name-not-confirming-to-format--%s", uniPortName))
	}
	...
	if t.resourceMgr.GetTechnology() == epon {
		...
		if err := t.addResourceInstanceToKVStore(ctx, tpID, uniPortName, resInst); err != nil {
			...
			if len(errList) > 0 {
				logger.Fatalw(ctx, "failed-to-free-up-resources-on-kv-store--system-behavior-has-become-erratic", log.Fields{"tpID": tpID, "uniPortName": uniPortName})
			}
			return nil, err
		}
...
func (t *TechProfileMgr) DeleteTechProfileInstance(ctx context.Context, tpID uint32, uniPortName string) error {
	...
	if !uniPortNameFormatRegexp.Match([]byte(uniPortName)) {
		logger.Errorw(ctx, "uni-port-name-not-confirming-to-format", log.Fields{"uniPortName": uniPortName})
		return errors.New(fmt.Sprintf("uni-port-name-not-confirming-to-format--%s", uniPortName))
	}
	path := t.GetTechProfileInstanceKey(ctx, tpID, uniPortName)
	t.tpInstanceMapLock.Lock()
	defer t.tpInstanceMapLock.Unlock()
...
func (t *TechProfileMgr) reconcileTpInstancesToCache(ctx context.Context) error {
	...
	tech := t.resourceMgr.GetTechnology()
	kvPairs, _ := t.config.ResourceInstanceKVBacked.List(ctx, tech)
