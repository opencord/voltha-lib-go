...
func (t *TechProfileMgr) GetTPInstanceFromKVStore(ctx context.Context, techProfiletblID uint32, path string) (interface{}, error) {
	...
	// is broken into ["service" "voltha" "technology_profiles" "XGS-PON" "64" "pon-{0}" "onu-{1}" "uni-{1}"]
	...
	case epon:
		resPtr = &KvEponIns
	default:
		log.Errorw("unknown-tech", log.Fields{"tech": pathSlice[3]})
		return nil, fmt.Errorf("unknown-tech-%s", pathSlice[3])
	}
...
func (t *TechProfileMgr) addTechProfInstanceToKVStore(ctx context.Context, techProfiletblID uint32, uniPortName string, tpInstance interface{}) error {
	var tpInstanceJson []byte
	var err error
	...
	case xgspon, xgsponBbsim, gpon:
		...
		if err != nil {
			logger.Errorw("Error in marshaling into Json format", log.Fields{"key": path, "tpinstance": tpInstanceTech})
		}
	case epon:
		tpInstanceEpon := tpInstance.(*EponProfile)
		tpInstanceJson, err = json.Marshal(*tpInstanceEpon)
		if err != nil {
			logger.Errorw("Error in marshaling into Json format", log.Fields{"key": path, "tpinstance": tpInstanceEpon})
		}
	default:
...
func (t *TechProfileMgr) getTPFromKVStore(ctx context.Context, techProfiletblID uint32, uniPortName string) interface{} {
	var kvtechprofile interface{}
	var defaultkvtechprofile DefaultTechProfile
	var defaultkveponprofile DefaultEponProfile
	path := t.GetTechProfileInstanceKVPath(techProfiletblID, uniPortName)
	// For example:
	// tpPath like "service/voltha/technology_profiles/XGS-PON/64/pon-{0}/onu-{1}/uni-{1}"
	// is broken into ["service" "voltha" "technology_profiles" "XGS-PON" "64" "pon-{0}" "onu-{1}" "uni-{1}"]
	...
	switch pathSlice[3] {
	case xgspon, xgsponBbsim, gpon:
		kvtechprofile = defaultkvtechprofile
	case epon:
		kvtechprofile = defaultkveponprofile
	default:
		logger.Errorw("unknown-tech", log.Fields{"tech": pathSlice[3]})
		return nil
	}
...
func (t *TechProfileMgr) CreateTechProfInstance(ctx context.Context, techProfiletblID uint32, uniPortName string, intfId uint32) (interface{}, error) {
	...
	if pathSlice[3] == epon {
		tp := t.getTPFromKVStore(ctx, techProfiletblID, uniPortName).(*DefaultEponProfile)
		if tp != nil {
		...
		tp := t.getTPFromKVStore(ctx, techProfiletblID, uniPortName).(*DefaultTechProfile)
		if tp != nil {
			if err := t.validateInstanceControlAttr(tp.InstanceCtrl); err != nil {
				logger.Error("invalid-instance-ctrl-attr--using-default-tp")
...
func (t *TechProfileMgr) allocateTPInstance(ctx context.Context, uniPortName string, tp *DefaultTechProfile, intfId uint32, tpInstPath string) *TechProfile {
	...
	if tp.InstanceCtrl.Onu == "multi-instance" {
		...
		if tpInst, err := t.getSingleInstanceTp(ctx, tpInstPath); err != nil {
			...
			tcontIDs = append(tcontIDs, tpInst.(*TechProfile).UsScheduler.AllocID)
		}
	}
...
func (t *TechProfileMgr) allocateEponTPInstance(ctx context.Context, uniPortName string, tp *DefaultEponProfile, intfId uint32, tpInstPath string) *EponProfile {
	...
	if tp.InstanceCtrl.Onu == "multi-instance" {
		...
		if tpInst, err := t.getSingleInstanceTp(ctx, tpInstPath); err != nil {
			...
			// Use the alloc-id from the existing TpInstance
			tcontIDs = append(tcontIDs, tpInst.(*EponProfile).AllocID)
		}
	}
...
func (t *TechProfileMgr) getSingleInstanceTp(ctx context.Context, tpPath string) (interface{}, error) {
	var tpInst interface{}
	var tpInstTech TechProfile
	var tpInstEpon EponProfile
	// For example:
	// tpPath like "service/voltha/technology_profiles/XGS-PON/64/pon-{0}/onu-{1}/uni-{1}"
	// is broken into ["service" "voltha" "technology_profiles" "XGS-PON" "64" "pon-{0}" "onu-{1}" "uni-{1}"]
	pathSlice := regexp.MustCompile(`/`).Split(tpPath, -1)
	switch pathSlice[3] {
	case xgspon, xgsponBbsim, gpon:
		tpInst = tpInstTech
	case epon:
	...
	default:
		logger.Errorw("unknown-tech", log.Fields{"tech": pathSlice[3]})
		return nil, errors.New("unknown-tech")
	}
	// For example:
