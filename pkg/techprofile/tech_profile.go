...
func (tpm *TechProfileMgr) GetTrafficQueues(tp *TechProfile, Dir tp_pb.Direction) ([]*tp_pb.TrafficQueue, error) {
	...
	if Dir == tp_pb.Direction_UPSTREAM {
		...
		for Count := 0; Count < NumGemPorts; Count++ {
			...
			if schedPolicy == -1 {
				...
				return nil, errors.New("Upstream Gem port traffic queue creation failed due to unrecognized scheduling policy")
			}
			...
			if discardPolicy == -1 {
				...
				return nil, errors.New("Upstream Gem port traffic queue creation failed due to unrecognized discard policy")
			}
		...
		for Count := 0; Count < NumGemPorts; Count++ {
			...
			if schedPolicy == -1 {
				...
				return nil, errors.New("Downstream Gem port traffic queue creation failed due to unrecognized scheduling policy")
			}
			...
			if discardPolicy == -1 {
				...
				return nil, errors.New("Downstream Gem port traffic queue creation failed due to unrecognized discard policy")
			}
	...
	return nil, errors.New("Downstream Gem port traffic queue creation failed due to unsupported direction")
}
