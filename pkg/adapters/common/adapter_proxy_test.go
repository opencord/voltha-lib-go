...
func TestSendInterAdapterMessage(t *testing.T) {
	...
	a := call.KvArgs[0].(*kafka.KVArg)

	b := a.Value.(*ic.InterAdapterMessage)

	assert.Equal(t, b.Header.Id, "testMessage")
	assert.Equal(t, b.Header.Type, ic.InterAdapterMessageType_DELETE_GEM_PORT_REQUEST)
	assert.Equal(t, b.Header.FromTopic, "Adapter1")
	assert.Equal(t, b.Header.ToTopic, "Adapter2")
	assert.Equal(t, b.Header.ToDeviceId, "testDeviceId")
	assert.Equal(t, b.Header.ProxyDeviceId, "testProxyDeviceId")

	assert.Equal(t, a.Key, "msg")
}
...
func TestHeaderId(t *testing.T) {
	...
	adapter.SendInterAdapterMessage(context.TODO(), delGemPortMsg, ic.InterAdapterMessageType_DELETE_GEM_PORT_REQUEST, "Adapter1", "Adapter2", "testDeviceId", "testProxyDeviceId", "")
	call := mockKafkaIcProxy.InvokeRpcSpy.Calls[1]
