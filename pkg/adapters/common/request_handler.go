...
func (rhp *RequestHandlerProxy) Get_value(args []*ic.Argument) (*voltha.ReturnValues,error) {
        if len(args) < 3 {
                logger.Warn("invalid-number-of-args", log.Fields{"args": args})
                return nil,errors.New("invalid-number-of-args")
        }
	...
	valuetype :=  &ic.IntType{}
        for _, arg := range args {
                ...
                case "id":
                        if err := ptypes.UnmarshalAny(arg.Value, id); err != nil {
                                logger.Warnw("cannot-unmarshal-transaction-ID", log.Fields{"error": err})
                                return nil,err
                        }
                case "valuetype":
		...
		default:
			logger.Warnw("DINESH Key not found", log.Fields{"arg.Key": arg.Key})
                }
        }

	logger.Debugw(" DINESH before invoke  Get_value", log.Fields{"pdeviceId": pDeviceId, "id": id, "valuetype": valuetype, "device": device})
        //Invoke the Get_value API on the adapter
        return  rhp.adapter.Get_value(pDeviceId.Val,device, id,voltha.ValueType_Type(valuetype.GetVal()))
}
