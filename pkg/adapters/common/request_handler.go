...
func (rhp *RequestHandlerProxy) Enable_port(args []*ic.Argument) error {
	log.Infow("enable-port", log.Fields{"args": args})
	return rhp.invokeEnableorDisablePort(args, true)
}

func (rhp *RequestHandlerProxy) Disable_port(args []*ic.Argument) error {
	log.Infow("disable-port", log.Fields{"args": args})
	return rhp.invokeEnableorDisablePort(args, false)
}

func (rhp *RequestHandlerProxy) invokeEnableorDisablePort(args []*ic.Argument, enabler bool) error {
	if len(args) < 3 {
	...
	if enabler == true {
		if err := rhp.adapter.Enable_port(device.Val, port); err != nil {
			log.Errorw("Error-occured-while-Enable-port", log.Fields{"error": err, "arguments": args})
			return status.Errorf(codes.NotFound, "%s", err.Error())
		}
	} else {
		if err := rhp.adapter.Disable_port(device.Val, port); err != nil {
			log.Errorw("Error-occured-while-Disable-port", log.Fields{"error": err, "arguments": args})
			return status.Errorf(codes.NotFound, "%s", err.Error())
		}
	}
	return nil
