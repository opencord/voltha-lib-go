...
func (rhp *RequestHandlerProxy) Start_omci_test(args []*ic.Argument) (*ic.TestResponse, error) {
	...
	for _, arg := range args {
		...
		case "omcitestrequest":
			if err := ptypes.UnmarshalAny(arg.Value, req); err != nil {
				logger.Warnw("cannot-unmarshal-omcitestrequest", log.Fields{"error": err})
				return nil, err
			}
		}
	}
	logger.Debugw("Start_omci_test", log.Fields{"deviceId": device.Id, "req": req})
	var result *ic.TestResponse
	var err error
	if result, err = rhp.adapter.Start_omci_test(device, req); err != nil {
		return nil, status.Errorf(codes.NotFound, "%s", err.Error())
	}
	return result, nil
}
