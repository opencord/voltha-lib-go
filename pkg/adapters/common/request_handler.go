...
func (rhp *RequestHandlerProxy) Get_ext_value(args []*ic.Argument) (*voltha.ReturnValues, error) {
	...
	logger.Debugw("before-invoke-adapter-get-ext-value", log.Fields{"pdeviceId": pDeviceId, "id": id, "valuetype": valuetype, "device": device})
	//Invoke the Get_value API on the adapter
	return rhp.adapter.Get_ext_value(pDeviceId.Val, device, id.Val, voltha.ValueType_Type(valuetype.Val))
}
