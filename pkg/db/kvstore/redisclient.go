...
/*
func (c *RedisClient) AcquireLock(ctx context.Context, lockName string, timeout time.Duration) error {
	var pool = goredis.NewPool(c.redisAPI)
	rs := redsync.New(pool)
	mutexname := "v-redis-" + lockName
	mutex := rs.NewMutex(mutexname)
	mutex.Lock()
	c.addLockName(lockName, mutex)
	return nil
}

func (c *RedisClient) addLockName(lockName string, lock *redsync.Mutex) {
	c.lockToMutexLock.Lock()
	defer c.lockToMutexLock.Unlock()
	c.lockToMutexMap[lockName] = lock
}

func (c *RedisClient) deleteLockName(lockName string) {
	c.lockToMutexLock.Lock()
	defer c.lockToMutexLock.Unlock()
	delete(c.lockToMutexMap, lockName)
}

func (c *RedisClient) getLock(lockName string) *redsync.Mutex {
	c.lockToMutexLock.Lock()
	defer c.lockToMutexLock.Unlock()
...
func (c *RedisClient) ReleaseLock(lockName string) error {
	mutexname := "v-redis-" + lockName
	lock := c.getLock(mutexname)
	var err error
	if lock != nil {
		if _, e := lock.Unlock(); e != nil {
			err = e
		}
	}
	c.deleteLockName(mutexname)
	return err
}
