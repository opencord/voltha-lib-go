...
func (c *RedisClient) Put(ctx context.Context, key string, value interface{}) error {
	...
	for _, cmd := range cmds {
		if cmd.Err() != nil && cmd.Err() != redis.Nil {
			logger.Warnw(ctx, "pipeline-command-failed", log.Fields{"error": cmd.Err(), "key": key})
			return cmd.Err()
		}
	}
	return nil
...
func (c *RedisClient) DeleteWithPrefix(ctx context.Context, prefixKey string) error {
	...
	for start < length {
		...
		for _, cmd := range cmds {
			if cmd.Err() != nil && cmd.Err() != redis.Nil {
				logger.Warnw(ctx, "pipeline-command-failed", log.Fields{"error": cmd.Err(), "key": prefixKey})
				return cmd.Err()
			}
		}
