...
// GetWithRetry retrieves a key-value pair from the KV store and retries on context deadline exceeded errors.
func (c *EtcdClient) GetWithRetry(ctx context.Context, key string) (*KVPair, error) {
	client, err := c.pool.Get(ctx)
	if err != nil {
		return nil, err
	}
...
// PutWithRetry writes a key-value pair to the KV store and retries on context deadline exceeded errors.
func (c *EtcdClient) PutWithRetry(ctx context.Context, key string, value interface{}) error {
	var val string
	var err error
	if val, err = ToString(value); err != nil {
		return fmt.Errorf("unexpected-type-%T", value)
...
// DeleteWithRetry removes a key from the KV store and retries on context deadline exceeded errors.
func (c *EtcdClient) DeleteWithRetry(ctx context.Context, key string) error {
	client, err := c.pool.Get(ctx)
	if err != nil {
		return err
	}
