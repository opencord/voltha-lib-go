...
const (
	...
	activityProbeInterval = 2 * time.Second
...
func newTestClient(apiEndpoint string, handler RestartedHandler) *testClient {
	...
	tc.client, err = NewClient(apiEndpoint,
		handler,
		ActivityCheck(true),
		ActivityProbeInterval(activityProbeInterval))
	if err != nil {
		return nil
	}
	return tc
...
func testNoActivity(t *testing.T) {
	...
	timer := time.NewTimer((activityProbeInterval + 1*time.Second) * time.Duration(numChecks))
	defer timer.Stop()
	count := 0
loop:
	for {
...
func testServerLimit(t *testing.T) {
	...
	for i := 1; i <= numRPCs; i++ {
		...
		go func(seq int) {
			...
			//} else {
			//	_, err = coreClient.ChildDevicesDetected(ctx, &common.ID{Id:strconv.Itoa(seq)})
			//}
			if err != nil {
				lock.Lock()
