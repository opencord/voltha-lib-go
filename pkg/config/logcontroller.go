...
func (c *ComponentLogController) persistInitialDefaultLogConfigs(ctx context.Context) {
	...
	if err != nil {
		log.Errorw("failed-to-retrieve-global-default-log-config-at-startup", log.Fields{"error": err})
	} else {
	...
	componentDefaultLogLevel, err := c.componentNameConfig.Retrieve(ctx, defaultLogLevelKey)
	if err != nil {
		log.Errorw("failed-to-retrieve-component-default-log-config-at-startup", log.Fields{"error": err})
	} else {
		if componentDefaultLogLevel == "" {
