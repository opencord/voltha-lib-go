...
func (c *ComponentLogController) processLogConfig() {
	...
	componentConfigEventChan := c.componentNameConfig.MonitorForConfigChange()

	globalConfigEventChan := c.GlobalConfig.MonitorForConfigChange()
	...
	for {
		...
		updatedLogConfig, err := c.buildUpdatedLogConfig()
		if err != nil {
		...
		err = c.loadAndApplyLogConfig(updatedLogConfig)
		if err != nil {
...
func (c *ComponentLogController) getGlobalLogConfig() (string, error) {
	...
	for globalKey, globalLevel := range globalLogConfig {
		if _, err := log.StringToLogLevel(globalLevel); err == nil && globalKey == "default" {
			...
			log.Warnw("unsupported-loglevel-config-defined-at-global-context-pacakge-name", log.Fields{"package-name": globalKey, "log-level": globalLevel})
		}
	}
...
func (c *ComponentLogController) buildUpdatedLogConfig() (map[string]string, error) {
	globalLogLevel, err := c.getGlobalLogConfig()
	if err != nil {
		log.Warnw("unable-to-retrieve-global-log-config", log.Fields{"error": err})
		return nil, err
	}

	componentLogConfig, err := c.getComponentLogConfig(globalLogLevel)
	if err != nil {
		log.Warnw("unable-to-retrieve-component-log-config", log.Fields{"error": err})
		return nil, err
	}
...
func (c *ComponentLogController) loadAndApplyLogConfig(logConfig map[string]string) error {
	...
	if err != nil {
		log.Warnw("unable-to-generate-log-config-hash", log.Fields{"err": err})
		return err
	}
