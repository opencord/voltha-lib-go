...
func (c *ComponentConfig) processKVStoreWatchEvents() {
	...
	log.Debugw("processing-kvstore-event-change", log.Fields{"ccKeyPrefix": ccKeyPrefix})
	...
	for watchResp := range c.kvStoreEventChan {
		...
		if watchResp.EventType == kvstore.CONNECTIONDOWN || watchResp.EventType == kvstore.UNKNOWN {
			log.Warnw("Received Invalid Change Type in Watch Channel from Kvstore", log.Fields{"changeType": watchResp.EventType})
			continue
		}
		...
		// Storeing default in configAttribute variable
		ky := fmt.Sprintf("%s", watchResp.Key)
		configAttribute := strings.TrimPrefix(ky, ccPathPrefix)
		configEvent := &ConfigChangeEvent{ChangeType, configAttribute, watchResp.Value}

		log.Debugw("Received Config Change event from backend", log.Fields{"changeEvent": configEvent})
