...
// Creates a Child Span from Parent Span embedded in passed context. Should be used before initiating
// any call in synchronous manner (without go keyword) in following conditions:
...
// 3. Any method which is suspected to be time consuming...
...
// Creates a Async Child Span from Parent Span embedded in passed context. Should be used right after initiating
// any call in asynchronous manner (with go keyword). The original context should be passed to go routine and this
// method should be called as first statement in go routine.
func CreateAsyncSpan(ctx context.Context, taskName string, keyAndValues ...Fields) (opentracing.Span, context.Context) {
	parentSpan := opentracing.SpanFromContext(ctx)

	// We should always be creating Aysnc span from a Valid parent span
	if parentSpan == nil {
		defaultLogger.Fatal(context.Background(), "Async soan must be created with a Valid parent span only")
	}

	asyncSpan, newCtx := opentracing.StartSpanFromContext(ctx, taskName, opentracing.FollowsFrom(parentSpan.Context()))

	if parentSpan.BaggageItem(RootSpanNameKey) == "" {
...
// Creates a New Background Context and copies over the Span from passed context into it.
...
// This will allow propagation of span with context.Background() as the base context (and not the original context)
func CopySpanIntoNewContext(ctx context.Context) context.Context {
	span := opentracing.SpanFromContext(ctx)
