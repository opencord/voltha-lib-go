...
var lfm LogFeaturesManager = LogFeaturesManager{}
...
func InitTracingAndLogCorrelation(tracePublishEnabled bool, traceAgentAddress string, logCorrelationEnabled bool) (io.Closer, error) {
	...
	tracer, _, err := constructJaegerTracer(tracePublishEnabled, traceAgentAddress, true)
	if err != nil {
		return nil, err
	}
...
func replaceActiveTracer(tracer opentracing.Tracer) {
	currentActiveTracer := opentracing.GlobalTracer()
	opentracing.SetGlobalTracer(tracer)
...
func SetLogCorrelationStatus(isEnabled bool) {
	...
	if isEnabled {
		...
		if _, ok := opentracing.GlobalTracer().(opentracing.NoopTracer); ok {
			tracer, _, err := constructJaegerTracer(lfm.isTracePublishingEnabled, lfm.activeTraceAgentAddress, false)
			if err != nil {
				defaultLogger.Warnf(context.Background(), "Log Correlation Enable operation failed with error: %s", err.Error())
				return
			}

			replaceActiveTracer(tracer)
		}
		...
		if _, ok := opentracing.GlobalTracer().(opentracing.NoopTracer); !ok && !lfm.isTracePublishingEnabled {
			replaceActiveTracer(opentracing.NoopTracer{})
		}
...
func SetTracePublishingStatus(isEnabled bool) {
	...
	if isEnabled {
		...
		tracer, _, err := constructJaegerTracer(isEnabled, lfm.activeTraceAgentAddress, false)
		if err != nil {
...
func CreateChildSpan(ctx context.Context, taskName string, keyAndValues ...Fields) (opentracing.Span, context.Context) {
	if !lfm.isLogCorrelationEnabled && !lfm.isTracePublishingEnabled {
